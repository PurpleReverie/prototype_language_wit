...
- `/tests/fixtures/08-additive-partials/` — 9 additive-partial fixtures + `_notes.md` + `cross-file-merge/` subdir (3 files). M1.08 merge `795de40`. Scenarios: `simple-additive-prefix.wit`, `block-additive.wit`, `single-line-additive.wit`, `multiple-additive-same-file.wit`, `cross-file-merge/`, `mix-normal-and-additive.wit`, `order-preservation.wit`, `mixed-body-shape.wit`, `additive-with-captures.wit`. **7 concrete proposals**: (1) expand-time merge; (2) base+additives compose into single merged node; (3) **SHAPE MUST MATCH** — mixed shapes error (revision of earlier "coerce to block" lean); (4) document-order depth-first traversal across files; (5) cross-file shared namespace; (6) captures forbidden on `+#x` (additives inherit base's capture list); (7) bare `+#x` (no body) is complete / no-op-merge.
...
- `/tests/fixtures/09-records/` — 10 record fixtures + `_notes.md` (551 lines). M1.09 merge `30b5381`. Scenarios: `inline-single-field.wit`, `inline-multi-field.wit`, `multi-line-record.wit`, `nested-record.wit`, `multi-word-key.wit`, `multi-word-value.wit`, `scalar-types.wit`, `trailing-comma.wit`, `empty-record.wit`, `comma-in-value.wit` (the last is treated as an ERROR fixture — comma always separates fields). **8 CONCRETE PROPOSALS**: (1) I.5 eager scalar recognition — number-shape `/^-?\d+(\.\d+)?$/` and lowercase booleans `true`/`false` recognized at lex-time in record-value slots only; (2) I.15 indentation in records is cosmetic (not required); (3) I.4 record-value side — comma ALWAYS separates record fields (no quoting, no inline-only carve-out) — `comma-in-value.wit` is an error fixture; (4) hyphen-as-separator parity with M1.05/M1.06 confirmed for record fields; (5) trailing comma `{ a - 1, }` parses identically to `{ a - 1 }`; (6) empty `{ }` = `Record{fields: []}` (and `{}` equivalent); (7) brace value WITHOUT preceding `-` — flagged; (8) lowercase exact boolean only within record-value context (composes with proposal 1).
...
**From `09-records/_notes.md` (M1.09):**
- **PLAN.md I.5 — scalar type distinction — CONCRETE PROPOSAL.** Eager lex-time recognition of number-shape (`/^-?\d+(\.\d+)?$/`) and lowercase boolean (`true` / `false`) tokens within record-value slots ONLY. Everything else (including bare `True`, `TRUE`, mixed-case) is a string. Strings remain unquoted by default.
- **PLAN.md I.15 — indentation in records — CONCRETE PROPOSAL.** Cosmetic (not required). Multi-line record fields may be indented for readability but indentation carries no semantic weight; closing `}` may appear at any column.
- **PLAN.md I.4 — record-value side — CONCRETE PROPOSAL.** Comma ALWAYS separates record fields. No quoting mechanism, no inline-vs-multi-line carve-out. `comma-in-value.wit` (`{ msg - hello, world }`) is treated as an ERROR fixture — author must split or rephrase. Keeps parsing rules uniform across record forms.
- **Hyphen-as-separator parity — CONCRETE PROPOSAL.** ` - ` (space-hyphen-space) as key/value separator carries over from M1.05/M1.06 unchanged into record fields. Bare `-` inside a word remains literal.
- **Trailing comma — CONCRETE PROPOSAL.** `{ a - 1, }` parses identically to `{ a - 1 }`; parity with M1.05 parens-form proposal.
- **Empty record — CONCRETE PROPOSAL.** `{ }` parses as `Record { fields: [] }`. `{}` (no inner whitespace) is equivalent. Not an error, not a no-op — a valid empty record literal.
- **Brace value without `-`** — flagged: a token sequence like `{ word }` (single bare token) — positional field, error, or string? New `I.review` item.
- **Boolean literal recognition** — strict lowercase `true` / `false` only; case-insensitive variants are strings. Reserved only within record-value context (composes with I.5 proposal). New `I.review`.

**From `08-additive-partials/_notes.md` (M1.08):**
- **Expand-time merge — CONCRETE PROPOSAL.** Parser emits `+#x` as a distinct AST shape (`NodeDef.additive = true`); resolver collects base + additives; expander performs the merge into a single materialized definition at expand time. Keeps parse/resolve stages pure.
- **Base + additives compose — CONCRETE PROPOSAL.** A single `#x` plus N `+#x` resolves to one merged node; absence of base `#x` with only `+#x` is an error (no implicit base). All additives MUST find a base.
- **Shape MUST match — CONCRETE PROPOSAL (revision).** Earlier lean was "coerce to block"; revised to STRICT EQUALITY — base shape (block / single-line / value-block) and all additives' shapes must match exactly. Mixed-shape merge is an error (`E_ADDITIVE_SHAPE_MISMATCH`). Cleaner diagnostics, no silent coercion.
- **Document-order depth-first — CONCRETE PROPOSAL.** Merge child order = document order across the resolved reference graph, depth-first traversal. Deterministic + intuitive for authors.
- **Cross-file shared namespace — CONCRETE PROPOSAL.** `#x` and `+#x` live in the same global namespace as referenced files; M1.07 I.9 "global within ref graph" extends to additives.
- **Captures forbidden on `+#x` — CONCRETE PROPOSAL.** `+#x ||a, b|| ...` is an error. Capture list lives only on base `#x`; additives inherit it. Avoids capture-list reconciliation across additives.
- **Bare `+#x` complete — CONCRETE PROPOSAL.** `+#x` with no body is a complete (no-op) additive that asserts membership in the merge graph without contributing content. Useful for ordering / index-only declarations.

**Accumulated load (M1.00–M1.09):** ~68+ surfaced `I.review` items across ten `_notes.md` files plus **~33 CONCRETE PROPOSALS + 1 LEAN** ready to ratify (I.6 from M1.04; I.12 + 4 from M1.05; pipe-side affirmation + 4 new from M1.06; 7 new + I.17 lean from M1.07; 7 new from M1.08 including shape-match revision; 8 new from M1.09 including I.5/I.15/I.4 record-value). M1.review will be substantial — schedule a dedicated review pass before M2 begins; expect spec v0.2 / PLAN.md amendments. **Memory pruning is overdue** — next memory update should compact per-category "Surfaced design questions" into one-line digests, or move them to a sibling `design-questions.md`.
...
- **Task 10 — M1.08 additive partials fixtures** (merge `795de40`): 9 root `.wit` fixtures + `cross-file-merge/` subdir (3 files) + `_notes.md` under `tests/fixtures/08-additive-partials/`. Scenarios: `simple-additive-prefix.wit`, `block-additive.wit`, `single-line-additive.wit`, `multiple-additive-same-file.wit`, `cross-file-merge/` (2-file cross-document merge probe), `mix-normal-and-additive.wit`, `order-preservation.wit`, `mixed-body-shape.wit`, `additive-with-captures.wit`. **Narration in `.wit` FORBIDDEN** (convention preserved). First contact with `+#name` additive-partial prefix; builds on M1.07 definition shapes (block / single-line / value-block) and PLAN section E.1 W7.2–W7.3 + DS-7. **7 CONCRETE PROPOSALS surfaced** (see "From `08-additive-partials/_notes.md`" above): expand-time merge; base+additives compose into one materialized def; **shape MUST match strictly (revision of earlier "coerce to block" lean)** — mixed shapes error; document-order depth-first traversal across resolved file graph; cross-file shared namespace (extends M1.07 I.9); captures forbidden on `+#x` (additives inherit base); bare `+#x` with no body is complete / no-op. Cross-file fixture exercises the multi-file scenario directly; full cross-file machinery still deferred to M1.15. Accumulated proposal total raised to ~25 concrete proposals + 1 lean.
- **Task 11 — M1.09 records fixtures** (merge `30b5381`): 10 `.wit` fixtures + `_notes.md` (551 lines) under `tests/fixtures/09-records/`. Scenarios: `inline-single-field.wit`, `inline-multi-field.wit`, `multi-line-record.wit`, `nested-record.wit`, `multi-word-key.wit`, `multi-word-value.wit`, `scalar-types.wit`, `trailing-comma.wit`, `empty-record.wit`, `comma-in-value.wit`. **Narration in `.wit` FORBIDDEN**. First contact with the record literal `{ ... }` as the right-hand side of `#name:` definitions; uses ` - ` key/value separator (M1.05/M1.06 parity) and comma-separated fields. PLAN section E.1 W6.1–W6.2 + DS-9. **8 CONCRETE PROPOSALS surfaced**: (1) I.5 eager scalar recognition — number-shape `/^-?\d+(\.\d+)?$/` + lowercase boolean recognized at lex-time within record-value slots only; (2) I.15 indentation in records is cosmetic (closing `}` may appear at any column); (3) I.4 record-value side — comma ALWAYS separates fields (`comma-in-value.wit` is an ERROR fixture, not a probe of quoting); (4) hyphen-as-separator parity confirmed for record fields; (5) trailing comma `{ a - 1, }` parses identically to `{ a - 1 }`; (6) empty `{ }` = `Record{fields: []}` (and `{}` equivalent); (7) brace value without preceding `-` — flagged; (8) lowercase exact boolean only within record-value context. Cross-references M1.07 definitions (record lives on right of `#name:`). Accumulated proposal total raised to ~33 concrete proposals + 1 lean across ten notes files.
- **Task 9 — M1.07 definitions fixtures** (merge `ee89b68`): 12 `.wit` fixtures under `tests/fixtures/07-definitions/` covering block definition (`#name ... name#`), single-line def (`#name: value !!`), captures + interpolation (`#name ||a, b|| ... ::a:: ...`), interpolation-only, body slot (`...`), multi-line value (`#name:\n ... \n!!`), captures+body-slot+interpolation, single-line def with captures, forward reference, definition referencing another definition, multi-capture-list, and body-slot-only. **First contact with the DEFINITION-side of the language; in-fixture narration FORBIDDEN.** Directly probes PLAN.md I.7 (`!!` greedy-parse risk), I.16 (body-slot positioning — repurposed from short-close), I.8 (forward references), I.9 (definition scope); leans on I.17 downstream. `_notes.md` surfaces **8 CONCRETE PROPOSALS + 1 LEAN**: (1) I.7 — context-only `!!` (terminator only inside open `#name:` value; literal in prose); (2) I.16 — body slot `...` at-most-once, any position; (3) I.8 — resolved-at-expand-time (subsumes full hoisting); (4) I.9 — global within ref graph (in-file fixture pins the trivial case); (5) `::name::` interpolation is context-only (literal outside definition body — parity with I.7 proposal); (6) `||...||` reserved for capture-list opener position ONLY — composes with M1.06's empty-`||` body-context carve-out into one unified rule; (7) definition shape classification by opener byte (`#name\n`=block, `#name: ... !!`=single-line, `#name:\n`=value-block); (8) lean (a) — `!!` is definition-side only (use-side close → error/literal). I.17 LEAN restated above. Accumulated total: ~60+ `I.review` items across eight notes files; **16 concrete proposals + 1 lean** carried into M1.review (count unchanged at the program level — see line below: total proposals across milestones are I.6 + 5 M1.05 + 4 M1.06 + 8 M1.07 = 18 proposals + I.17 lean. Note: this update reconciles the count — prior memory undercounted M1.07 by lumping the I.17 lean and treating shape classifier as a sub-bullet).
...
- **M1.10 dispatched — collections fixtures.** Branch `m1-10-collections-fixtures`; single commit message `M1.10: collections fixtures ([] of values/records, nested)`. PLAN section E.1 W6.3–W6.4 + DS-9. **Narration in `.wit` FORBIDDEN**. First contact with the collection literal `[ ... ]` as the right-hand side of `#name:` definitions or as inline values. Builds on M1.09 records: a collection may contain scalar values, records, or nested collections. Item separation question is the central probe — comma vs whitespace vs newline.
  - Scenarios briefed (kebab-case, one purpose each):
    - `inline-values.wit` — `#themes: [a, b, c]` inline scalar collection.
    - `inline-records.wit` — `[{a-1}, {a-2}]` inline collection of records (M1.09 parity).
    - `multi-line-values.wit` — `[\n  a\n  b\n]` multi-line scalar items (probes newline-as-separator).
    - `multi-line-records.wit` — `[\n  { ... }\n  { ... }\n]` multi-line records.
    - `nested-collection.wit` — `[[1,2], [3,4]]` exercises bracket-balancing.
    - `multi-word-items.wit` — `[moral failure, attention]` multi-word values across comma boundaries.
    - `mixed-types.wit` — `[1, "two", true]` (note: this fixture probes whether quoted strings are even a thing in collections — flag as I.review if quoting is foreign).
    - `trailing-comma.wit` — `[a, b, c,]` parity with M1.05/M1.09 trailing-comma proposal.
    - `empty-collection.wit` — `[ ]` empty list semantics.
    - `whitespace-separator.wit` — `[ a b c ]` PROBE: are spaces alone enough to separate items, or is comma required?
  - Create `tests/fixtures/10-collections/_notes.md`. **No narration in `.wit`.** Every H2 cites PLAN.md `I.x` or flags new `I.review`. Expected H2 sections:
    - **PLAN.md I.5 — type recognition parity with records.** Eager scalar recognition extends from records to collection items (numbers / booleans / strings same rules).
    - **PLAN.md I.15 — indentation in collections.** Cosmetic vs required (parity with M1.09 records).
    - **PLAN.md I.4 — comma vs spaces for items.** Central probe: is comma required, optional (whitespace also separates), or are newlines the multi-line separator while commas separate inline items? Implementer commits to one rule.
    - **Empty `[ ]` semantics** — parity with M1.09 empty `{ }`.
    - **Trailing comma parity** — affirm M1.05/M1.09 proposal.
    - **Mixed-type legality** — can a single collection mix numbers, strings, booleans, records? Or homogeneity required?
    - **Nested-collection bracket-balancing** — well-nested `]` matches innermost `[`; whitespace between adjacent `]` `]` significance.
    - **Whitespace-separator vs comma-required** — direct probe via `whitespace-separator.wit`; commit to spaces-suffice, comma-required, or context-dependent.
  - Avoid: definitions beyond `#name:` left-hand wrapper, scripting/`each`/`if` (those are M1.13+), `@x` references inside collection items (composition probe deferred). Use plain prose + `#name:` definitions only.
  - Do NOT add snapshot `.json` files — snapshot runner has not landed.
  - **Branch:** `m1-10-collections-fixtures`. **Single commit message:** `M1.10: collections fixtures ([] of values/records, nested)`.
- **Memory pruning still OVERDUE** (file now ~330+ lines). Next memory update should compact per-category "Surfaced design questions" into one-line digests, or move them to a sibling `design-questions.md` and keep this file the high-level index.
- **M1.review preparation** — surface area: ~33 concrete proposals + 1 lean across PLAN.md I.4, I.5, I.6, I.7, I.8, I.9, I.11, I.12, I.15, I.16, I.17 plus ~10 new composite I.review rules (capture-list reservation, interpolation context, shape classification, last-one-wins two-pass, additive-partial shape-strict-match, eager scalar recognition, record-field comma-always-separates, etc.). Before M2 begins, schedule a dedicated M1.review pass producing (1) spec v0.2 update, (2) PLAN.md I.x resolutions, (3) refreshed open-questions list for resolver/expander phase.

## (Archived) Notes from M1.09 cycle

- **M1.09 dispatched — records fixtures.** Branch `m1-09-records-fixtures`; single commit message `M1.09: records fixtures ({} inline/multi-line/nested)`. PLAN section E.1 W6.1–W6.2 + DS-9. **Narration in `.wit` FORBIDDEN** (convention now hard-locked from `03-comments/` onward). First contact with the record literal `{ ... }` — a set of named values used as definition values, distinct from the `(...)` parameter list and the `|...|` slot. Grounding: `/examples/10-records.wit` (this file is the source-of-truth narrative). Implementer builds on M1.07 definitions: a record lives on the right of `#name:` and uses the same ` - ` key/value delimiter as M1.05/M1.06 (parity probe).
  - Scenarios briefed (kebab-case, one purpose each):
    - `inline-single-field.wit` — `#x: { a - 1 }` simplest inline form.
    - `inline-multi-field.wit` — `#x: { a - 1, b - 2 }` comma-separated multi-field.
    - `multi-line-record.wit` — `#x: {\n  a - 1\n  b - 2\n}` newline-separated fields, indented body, closing `}` flush left.
    - `nested-record.wit` — `#x: { a { b - 1 } }` record value containing a sub-record.
    - `multi-word-key.wit` — `{ years at post - 31 }` exercises the M1.05/M1.06 ` - ` key/value delimiter for multi-word keys.
    - `multi-word-value.wit` — `{ name - Aldous Vane }` multi-word value parity.
    - `scalar-types.wit` — strings, numbers, booleans in one record (`{ name - Aldous Vane, years - 31, lamp lit - true }`); probes PLAN.md I.5 (numeric literal type distinction).
    - `trailing-comma.wit` — `{ a - 1, }` parity with M1.05's tolerated-trailing-comma proposal.
    - `empty-record.wit` — `{ }` (and consider `{}`) — empty record literal: valid empty set, no-op, or error?
    - `comma-in-value.wit` — `{ msg - hello, world }` — does the comma separate fields or belong to the value? Probes PLAN.md I.4 quoting necessity from the record-value side.
  - Create `tests/fixtures/09-records/_notes.md`. **No narration in `.wit` files.** Every H2 cites PLAN.md `I.x` or flags new `I.review`. Expected questions to probe / propose:
    - **PLAN.md I.5 — scalar type distinction.** Concrete proposal candidate: lex-time recognition of number-shape (`/^-?\d+(\.\d+)?$/`) and boolean-shape (`true`/`false`) tokens within record-value slots only; everything else is a string. Strings remain unquoted by default. Implementer must commit or diverge.
    - **PLAN.md I.15 — indentation in records/collections.** Multi-line records: is indentation REQUIRED for the body lines, or cosmetic? Closing `}` position rule. Concrete proposal candidate.
    - **PLAN.md I.4 — parameter value quoting** — record-value side. The `comma-in-value.wit` scenario forces the question: does `hello, world` parse as one value with a literal comma, or split at the comma into `msg - hello` + bare `world`? Three options: comma always-splits (require escape/quote for literal comma), comma-splits-only-on-inline-form (newline-separated multi-line has no comma issue), or introduce a quoted-string slot. New `I.review` formalizing.
    - **Multi-word key delimiter parity (`-` with surrounding spaces)** — affirm M1.05/M1.06 hyphen-as-separator proposal extends to record fields. New `I.review`.
    - **Trailing comma parity** — affirm M1.05 proposal: `{ a - 1, }` parses identically to `{ a - 1 }`. New `I.review` if any divergence emerges.
    - **Empty record `{ }` semantics** — valid empty set, no-op, or error? New `I.review`. Also: is `{}` (no inner whitespace) equivalent?
    - **Nested record termination** — `{ a { b - 1 } }`: must inner `}` precede outer `}` strictly (well-nested), and does whitespace between adjacent `}` `}` matter? New `I.review`.
    - **Boolean literal recognition** — only lowercase `true`/`false`, or case-insensitive? Reserved words inside record values only, or globally? Composes with PLAN.md I.5 proposal. New `I.review`.
    - **String values containing reserved chars** — `{` `}` `,` ` - ` inside a value require what? Composes with I.4 record-value-side. New `I.review`.
    - **Record inside parameter list** — out of scope for this milestone (no `@x({...})` fixtures). Note for future I.review.
  - Avoid: collections (`[ ]` list literals — that's M1.10), data access (`@x.field` already covered M1.04, but no record-field deref here), conditionals, iteration. Use plain prose + `#name:` definitions only.
  - Do NOT add snapshot `.json` files — snapshot runner has not landed.
  - **Branch:** `m1-09-records-fixtures`. **Single commit message:** `M1.09: records fixtures ({} inline/multi-line/nested)`.
