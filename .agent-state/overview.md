...
- **M1.08 dispatched — additive partials fixtures.** Branch `m1-08-additive-partials-fixtures`; commit `M1.08: additive-partials fixtures (+#name merge, edges)`. PLAN section E.1 W7.2–W7.3 + DS-7. **Narration in `.wit` FORBIDDEN.** First contact with `+#name` additive-partial prefix; implementer builds on M1.07 definitions (block, single-line, value-block) and probes how `+#name` merges with an existing `#name`. Scenarios briefed: `+#name` simple prefix; block additive `+#name body name#`; single-line additive `+#name: value !!`; multiple `+#bibliography` in one file (merging semantics); cross-file merge (two referenced files each adding `+#bibliography`); interaction between normal `#x` and additive `+#x` of the same name (does the mix error?); order preservation (children in document order across files?); non-mergeable body type (`+#x` block + `+#x` single-line — error?); additive partial with captures (legal?). Cross-reference `examples/16-additive-partials/` for grounding.
- **Memory pruning still OVERDUE** (file now ~280 lines). Next memory update should compact per-category "Surfaced design questions" into one-line digests, or move them to a sibling `design-questions.md` and keep this file the high-level index.
- **M1.review preparation** — surface area: ~18 concrete proposals + 1 lean across PLAN.md I.4, I.6, I.7, I.8, I.9, I.11, I.12, I.16, I.17 plus ~7 new composite I.review rules (capture-list reservation, interpolation context, shape classification, last-one-wins two-pass, etc.). Before M2 begins, schedule a dedicated M1.review pass producing (1) spec v0.2 update, (2) PLAN.md I.x resolutions, (3) refreshed open-questions list for resolver/expander phase.
